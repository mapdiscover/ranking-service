...
def setupDB(APIconf):
	tables = {"users": "createtable", "groups": "createtable"}
	cmds = {"createdb": """psql -c "CREATE DATABASE {};" """, "exeSQL": "psql --dbname={} --file={}", "createextension": "CREATE EXTENSION {};"}
	...
	if "db_extensions" in config:
		print("creating extensions...")
		for ext in config["db_extensions"]:
			print("  {}...".format(ext))
			dbhelper.executeCMD(sqls["createextension"].format(ext))
	...
	for i in tables:
		if dbhelper.tableExists(i):
			...
			if "table_" + i in APIconf:
				...
				dbhelper.executeCMD(sqls["createtable"].format(i, ",\n".join(APIconf["table_" + i])))
			else:
